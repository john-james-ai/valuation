...
tgr_range = np.arange(ASSUMPTIONS['terminal_growth_rate'] - 0.01, 
                      ASSUMPTIONS['terminal_growth_rate'] + 0.015, 0.005)
...
ax4.bar(dcf_df['year'], dcf_df['pv_fcf'] / 1_000_000, color='purple', alpha=0.7, label='Forecast Period')
ax4.bar([terminal_year], [pv_terminal_value / 1_000_000], color='orange', alpha=0.7, label='Terminal Value')
...
ax5.bar(range(len(bridge_labels)), bridge_values, color=['purple', 'orange', 'darkgreen'], alpha=0.7)
...
sns.heatmap(sensitivity_table / 1_000_000, annot=True, fmt='.1f', cmap='RdYlGn', 
            ax=ax6, cbar_kws={'label': 'EV ($M)'})
...
ax7.plot(dcf_df['year'], dcf_df['ebit'] / dcf_df['revenue'] * 100, 
         marker='o', color='steelblue', linewidth=2, label='Operating Margin')
ax7.axhline(y=ASSUMPTIONS['operating_margin'] * 100, color='red', 
            linestyle='--', alpha=0.5, label='Assumed Margin')
...
ax8.plot(dcf_df['year'], dcf_df['discount_factor'], marker='o', 
...
ax9.plot(company_revenue_weekly['ds'], company_revenue_weekly['revenue'] / 1000, 
...
plt.suptitle(f'DCF Valuation Dashboard - Enterprise Value: ${enterprise_value/1_000_000:.1f}M (as of {VALUATION_DATE.date()})', 
